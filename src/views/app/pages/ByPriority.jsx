            ...
            <div style={{overflow:"hidden"}} className="row gap-2 justify-center">
                ...
                <div onDrop={(e)=> onDrop(e, 0)} onDragOver={(e) => onDragOver(e) } className="col-lg-3 bg-blue-700 rounded-lg p-4 col-sm-6">
                    ...
                    {showInfoTask.includes(0) &&
                        baixa.map((task)=>{
                            return (
                                <Task onDragOver={(e)=> onDragOver(e)}  onDrag={(e)=> onDrag(e, task.id)} key={task.id + task.title} done={done} onClick={showDetails} className={`${themeCtx.theme == 'dark' ? 'dark':'bg-light'} `} detailsShow={detailsShow}  task={task} />
                                )
                            })
                        }
                </div>
                ...
                <div onDrop={(e)=> onDrop(e, 1)} onDragOver={(e) => onDragOver(e) } className="col-lg-3 overflow-y-auto  bg-yellow-700 p-4 col-sm-6 rounded-lg">
                    ...
                    {showInfoTask.includes(1) && media.map((task)=>{
                        return (
                            <Task onDragOver={(e)=> onDragOver(e)} onDropTask={(e) => onDropTask(e, task.id)} onDrag={(e)=> onDrag(e, task.id)} key={task.id + task.title} done={done} onClick={showDetails} className={`${themeCtx.theme == 'dark' ? 'dark':'bg-light'} `} detailsShow={detailsShow}  task={task} />
                        )
                    })}
                </div>
                }
                {showOnly.includes(2) &&
                <div onDrop={(e)=> onDrop(e, 2)} onDragOver={(e) => onDragOver(e) } className="col-lg-3 overflow-x-auto p-4 col-sm-6 bg-red-600">
